...
import LoadingAnimation from '../posts/loadingBar';
//import actors from './actors';

//import CommentList from './comments/commentList';
//import CommentBar from './comments/commentBar';

const AdminPosts = ({ profile, username, userId, groupId, posts, loading, commentText, handleCommentChange, handleAddComment, comments, allGroupLikes, }) => {  
    ...
    <div className='flex flex-col items-center'>
      ...
      posts.slice(0, visiblePosts).map((post, index) => (
        <div key={post.id} className={`${styles.post_box} ${post.up_down === "up" ? styles.hi_post : post.up_down === "down" ? styles.lo_post : ""}`}>
          <div className="w-full h-8 relative border-b border-slate-100 flex items-center"> 
            
              <button className="h-full w-1/3 relative flex justify-center items-center hover:bg-stone-200 rounded-tl-lg">Review</button>
              <button className="h-full w-1/3 relative flex justify-center items-center hover:bg-stone-200">Edit</button>
              <button className="h-full w-1/3 relative flex justify-center items-center hover:bg-stone-200 rounded-tr-lg">Delete</button>
          </div>
